...
# ✅ Correct CORS setup (Frontend hosted on Render)
...
class Settings(db.Model):
    ...
    idle_timeout = db.Column(db.Integer)
...
def admin_login():
    ...
    if admin and check_password_hash(admin.password, data['password']):
        token = create_access_token(identity={'type': 'admin', 'id': admin.id})
        return jsonify({'token': token, 'user': {'type': 'admin', 'username': admin.username}})
...
def employee_login():
    ...
    if emp and check_password_hash(emp.password, data['password']):
        token = create_access_token(identity={'type': 'employee', 'id': emp.id})
        return jsonify({'token': token, 'user': {'type': 'employee', 'username': emp.username, 'name': emp.name}})
...
if __name__ == '__main__':
    ...
    print("✅ Database ready. Running Flask app...")
